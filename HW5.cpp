...
void doubly_linked_list::insertion_sort() {
	...
	shared_ptr<node> p1 = head -> next;

	while (p1) {
		shared_ptr<node> temp = p1 -> next;
		if (p1 -> value < (p1 -> previous).lock() -> value) {
			if (tail == p1) {
				tail = (p1 -> previous).lock();
			}
            ...
            while (p2) {
                if (p2 -> value > p1 -> value) {
					...
					if (p1 -> next) {
						p1 -> next  -> previous = (p1 -> previous).lock();
					}
					if (head == p2) {
						head = p1;
					}	
					p1 -> previous = (p2 -> previous).lock();
					if ((p2 -> previous).lock()) {
